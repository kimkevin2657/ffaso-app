...
import { MEMBER_SCHEDULE_COLOR } from '../../constants/constants';

const window_height = Dimensions.get('window').height;

const Dates = ['일', '월', '화', '수', '목', '금', '토'];

const CalendarCustom = ({ navigation, schedules }) => {
                          ...
                          if (i < 3) {
                            return (
                              <PlanBox
                                key={schedule.id}
                                backgrounds={
                                  MEMBER_SCHEDULE_COLOR[schedule.type]
                                }
                              >
                                <PlanText
                                  numberOfLines={1}
    ...
    <CalendarContainer>
      ...
      <DateContainer>
        {Dates.map((date, index) => (
          <Days key={index} isSunday={index === 0} isSaturday={index === 6}>
            {date}
          </Days>
        ))}
      </DateContainer>
