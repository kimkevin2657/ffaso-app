...
const ResetPassword = ({ navigation }) => {
  ...
  const onRequestVerifyCode = async () => {
    ...
    try {
      const { data } = await api.get(
        `sms-send?scope=FIND_PASSWORD&email=${id}&phoneNumber=${
  ...
  const getNewPassword = async () => {
    ...
    let body = { email: id, phoneNumber: firstPhoneNumber + phoneNumber };
    try {
      const { data } = await api.post('users/findPassword/', body);
  ...
  return (
    <View style={styles.rootContainer}>
      ...
      />
      <NormalBoldLabel text={'아이디'} style={styles.title} />
...
const GoLoginModal = (props) => {
  const navigation = useNavigation();
  return (
    <ModalFrame visible={props.visible}>
      {/*<ColumnView style={modalStyle.contentPosition}>*/}
      <NormalBoldLabel
        text={'인증된 휴대폰 번호로\n' + '임시 비밀번호가 전송되었습니다.'}
        style={{ textAlign: 'center', fontSize: 18, lineHeight: 24 }}
      />
