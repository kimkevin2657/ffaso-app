...
export default function CertificateModify({ navigation, route }) {
  ...
  const [certificateTitle, setCertificateTitle] = useState(content.name);
  ...
  const SaveCertificate = async () => {
    ...
    try {
      ...
      let { data } = await api.patch('teacher-infos', body, {
        headers: {
  ...
  return (
    <View style={{ flex: 1, marginBottom: 25 }}>
      <ScrollView style={{ flex: 1 }}>
        <HeaderBar style={{ lineHeight: 100 }} text='자격증/수상 수정' />
